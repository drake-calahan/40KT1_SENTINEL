...
-> les seul machine qu'on veut surveiller sont les patator: tower et standby, les autre sont des poste client admin/SRE
...
> manque.
-> Surveillance **hôte** (serveur) + événements Docker en phase 1 ; Falco/eBPF
   conteneur en phase 4 **si la phase 1 montre qu'il manque** — pas descopé
   définitivement.
...
-> oui objectif apprentissage et mise ne pratique des bests practice finest tool du moment
---
...
-> **(a) `patator-standby`**, profil frugal (zéro budget). Plan B : **`bluefin`
   temporaire jour** pour le MVP si le standby s'avère impraticable ; bascule
   **hôte de secours 24/7** uniquement en **pire cas**.
...
-> **Zéro budget** — aucun achat matériel. Profil frugal obligatoire ; ~250 €
   documenté comme option future.
...
-> oui a moment exceptionnel derogatio exceptionnelle autorisé
...
-> que tailet on separe fortement l'exposé du SRE
...
> **noté comme dette**.
->ca sera une Dette, il faudrait, mais c'est un projet amateur sans enjeux financier pour une intrusion. pas prioritaire
---
...
-> Vue « sécurité » dans le **cockpit SRE** (ops) en P3 — OK. Console / labo
   **SecOps** = intention distincte ; si console dédiée, elle vit sur `bluefin`,
   sans confondre avec le cockpit ops.
...
->oui, je passe 5 min jour a regarder et 1h semaine de repassage
---
...
-> ok — harnais `.agent/` adapté : Claude Code + Cursor uniquement, **aucun
   serveur MCP**.
...
->ok on ne pense pas etre ci heavy sur ce projet une fois le mvp lancé
